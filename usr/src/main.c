...
int main(int argc, char* argv[]) {
	LogWrite(ALL, INF, "scut built at: " __DATE__ " " __TIME__);
	LogWrite(ALL, INF, "#######################################");
	int client = 1;
	int ch, tmpdbg;
	...
	while ((ch = getopt_long(argc, argv, "u:p:i:n:H:s:c:T:h:E:Q:D::o",
		...
		case 's':
			...
			break;
		case 'T':
			if ((sscanf(optarg, "%hhu:%hhu", &a_hour, &a_minute) != 2) || (a_hour >= 24)
				|| (a_minute >= 60)) {
	...
	if ((client != LOGOFF) && !((UserName && Password && UserName[0]
		&& Password[0]))) {
	...
	while (1) {
		...
		if (ret == 1) {
			...
			if (((int)a_hour * 60 + a_minute) > ((int)(cltime->tm_hour) * 60 + cltime ->
				tm_min)) {
				LogWrite(ALL, INF, "Waiting till %02hhd:%02hhd. Have a good sleep...", a_hour,
					a_minute);
				if (OfflineHookCmd) {
					system(OfflineHookCmd);
				}
				sleep((((int)a_hour * 60 + a_minute) - ((int)(cltime->tm_hour) * 60 + cltime
					->tm_min)) * 60 - cltime->tm_sec);
			}
			else {
				break;
			}
